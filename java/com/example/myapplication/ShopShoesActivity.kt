...
class ShopShoesActivity : AppCompatActivity() {
    ...
    private fun fetchProducts() {
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                val products = RetrofitInstance.api.shopShoes()
                withContext(Dispatchers.Main) {
                    productListAdapter.updateProducts(products)
                }
            } catch (e: Exception) {
                withContext(Dispatchers.Main) {
                    Toast.makeText(this@ShopShoesActivity, "Failed to load products", Toast.LENGTH_SHORT).show()
                }
            }
        }
    }
